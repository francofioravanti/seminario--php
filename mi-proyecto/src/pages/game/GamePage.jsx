...
// Add this function after getAuthHeaders (around line 30)
...
// Remove these lines completely - they're causing the token error:
// console.log('Headers que se enviarán:', {
//   'Authorization': `Bearer ${token}`,
//   'Content-Type': 'application/json'
// });
// console.log('URL completa:', 'http://localhost:8000/partidas');
// console.log('Método:', 'POST');

// Replace the existing guardarPartida function (around line 95-120)
const guardarPartida = async (resultado) => {
  try {
    const token = localStorage.getItem('token');
    const userId = localStorage.getItem('userId');
    ...
    const datosPartida = {
      mazo_id: parseInt(mazoId),
      el_usuario: resultado // 'gano', 'perdio', o 'empato'
    };

    console.log('Enviando datos de partida:', datosPartida);
    console.log('Headers:', {
      'Authorization': `Bearer ${token}`,
      'Content-Type': 'application/json'
    });

    const response = await axios.post('http://localhost:8000/partidas', datosPartida, {
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json'
      }
    ...
    console.log('Partida guardada exitosamente:', response.data);
  } catch (error) {
    console.error('Error completo al guardar partida:', error);
    console.error('Response data:', error.response?.data);
    console.error('Response status:', error.response?.status);
    console.error('Response headers:', error.response?.headers);
    
    if (error.response?.status === 403) {
      console.error('Error de permisos - verifica tu token de autenticación');
      console.error('Token actual:', localStorage.getItem('token'));
      console.error('UserId actual:', localStorage.getItem('userId'));
    }
  }
};
